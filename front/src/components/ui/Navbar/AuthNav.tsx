  ...
  if (isAuth) {
    return (
      <div className="flex items-center justify-end space-x-3 md:order-2 md:space-x-0 rtl:space-x-reverse">
        ...
        {Boolean (total) && (<span className="absolute text-xs font-bold -top-3 left-4 text-secondary_yellow-500"> 
          {total}
           </span>)}
        </div>
        </Link>
        ...
        <button onClick={logout} className="text-xl text-secondary_yellow-500" > <FiLogOut /> </button>
      </div>
    );
  }
